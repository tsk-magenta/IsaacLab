...
def main():
    ...
    # Initialize keyboard_stopped_count for use with the painted termination function
    ...
    def create_teleop_device(device_name: str, env: gym.Env):
        ...
        if device_name == "keyboard":
            return Se3Keyboard(pos_sensitivity=0.2, rot_sensitivity=0.5, env=env)
    ...
    env.sim.reset()
    env.reset()
    # Initialize keyboard_stopped_count for teleop reset function
    if not hasattr(env, "keyboard_stopped_count"):
        env.keyboard_stopped_count = 0
    
    # Reset the teleop interface which will clean up any existing particles
    print("=== Initial Teleop Reset ===")
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running():
            ...
            if running_recording_instance:
                ...
                if current_time - last_print_time >= 1.0:
                    ...
                    if isinstance(obv[0], dict) and "policy" in obv[0] and "eef_pos" in obv[0]["policy"]:
                        ...
                        # 1초마다 출력하도록 수정 필요 시 이전 답변 참고
                        # print(f"EEF World Pos: [X={eef_world_pos[0]:.3f}, Y={eef_world_pos[1]:.3f}, Z={eef_world_pos[2]:.3f}]")

                    # --- EEF와 MyBlock 타겟 간 거리 출력 (수정) ---
                    # obs_key_for_distance 변수에 설정 파일(PolicyCfg)에 정의한 거리 값 ObsTerm 이름 사용
                    obs_key_for_distance = "eef_to_myblock_dist" # <-- 실제 정의한 이름 사용!

                    if isinstance(obv[0], dict) and "policy" in obv[0] and obs_key_for_distance in obv[0]["policy"]:
                        # 바로 거리 텐서에 접근
                        distance_tensor = obv[0]["policy"][obs_key_for_distance]
                        # 텐서에서 스칼라 값 추출 (형태가 (1, 1) 가정, 아니면 [0].item() 시도)
                        distance = distance_tensor[0, 0].item()
                        # 1초마다 출력하도록 수정 필요 시 이전 답변 참고
                        # print(f"EEF to MyBlock Target Dist: {distance:.4f}")
                    # --------------------------------------------

                    # --- Subtask 'approach_1' 만족 여부 출력 (별도 처리) ---
                    # obs_key_for_proximity_bool 변수에 설정 파일(SubtaskCfg)에 정의한 bool 값 ObsTerm 이름 사용
                    obs_key_for_proximity_bool = "approach_1"
                    group_name_for_proximity_bool = "subtask_terms" # 정의된 그룹 이름 확인 필요

                    if isinstance(obv[0], dict) and group_name_for_proximity_bool in obv[0] and obs_key_for_proximity_bool in obv[0][group_name_for_proximity_bool]:
                        # 바로 boolean 텐서에 접근
                        is_near_tensor = obv[0][group_name_for_proximity_bool][obs_key_for_proximity_bool]
                        # 텐서에서 스칼라 값 추출 (형태가 (1, 1) 가정, 아니면 [0].item() 시도)
                        is_target_near = is_near_tensor[0, 0].item()
                        # 1초마다 또는 True일 때만 출력하도록 수정 가능
                        if is_target_near:
                            print(f"Subtask '{obs_key_for_proximity_bool}' met: True")
                    # ---------------------------------------------------
                ...
                if success_term is not None:
                    # Safely handle the success term result - it might return a boolean or a tensor
                    success_result = success_term.func(env, **success_term.params)
                    
                    # Convert to boolean based on result type
                    is_success = False
                    if isinstance(success_result, bool):
                        is_success = success_result
                    elif isinstance(success_result, torch.Tensor):
                        if success_result.numel() > 0:  # Check if tensor is not empty
                            is_success = bool(success_result[0])
                        else:
                            is_success = bool(success_result)
                    else:
                        # Try a generic boolean conversion as fallback
                        is_success = bool(success_result)
            ...
            if should_reset_recording_instance:
                # First, reset keyboard to clear particles and update counters
                if hasattr(env, "keyboard_stopped_count"):
                    previous_count = env.keyboard_stopped_count
                
                # Reset the teleop interface which will clean up particles and reset counts
                teleop_interface.reset()
                
                # Then reset simulation and environment
                env.sim.reset()
                env.recorder_manager.reset()
                env.reset()
                
                # Display count change information
                if hasattr(env, "keyboard_stopped_count"):
                    print(f"✅ Reset: Particles count {previous_count} → {env.keyboard_stopped_count}")

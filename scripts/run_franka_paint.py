...
"""Run a Franka Paint environment."""
...
def parse_args():
    """Parse arguments."""
    parser = argparse.ArgumentParser(description="Run a Franka robot painting environment.")
    parser.add_argument(
        "--num_envs", type=int, default=1, help="Number of environments. Defaults to 1."
    )
    parser.add_argument(
        "--task", type=str, default="Isaac-Paint-Franka-v0", help="Task name. Defaults to Isaac-Paint-Franka-v0."
    )
    parser.add_argument(
...
def main():
    ...
    for env_id in gym.registry.keys():
        if "Paint" in env_id and "Franka" in env_id:
            print(f"  - {env_id}")
    print(f"\nSelected environment: {args.task}")

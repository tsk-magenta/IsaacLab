...
def ee_moved(
    ...
    """Check if an end-effector is moved to the starting point by the specified robot."""
    ...
    moved = torch.logical_and(
        pose_diff < diff_threshold,
        torch.abs(robot.data.joint_pos[:, -1] - gripper_open_val.to(env.device)) > gripper_threshold,
    )
    moved = torch.logical_and(
        moved, torch.abs(robot.data.joint_pos[:, -2] - gripper_open_val.to(env.device)) > gripper_threshold
    )
...
def object_painted(
    ...
    """Check if an object is painted by the specified robot."""
    ...
    painted = torch.logical_and(xy_dist < xy_threshold, (height_dist - height_diff) < height_threshold)

    painted = torch.logical_and(
        torch.isclose(robot.data.joint_pos[:, -1], gripper_open_val.to(env.device), atol=1e-4, rtol=1e-4), painted
    )
    painted = torch.logical_and(
        torch.isclose(robot.data.joint_pos[:, -2], gripper_open_val.to(env.device), atol=1e-4, rtol=1e-4), painted
    )
